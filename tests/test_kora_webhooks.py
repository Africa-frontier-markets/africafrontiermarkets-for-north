...
class FakeDb:
    ...
    async def execute(self, _query):
        return FakeResult(self.records[0] if self.records else None)


@pytest.mark.asyncio
async def test_kora_signature_uses_data_object(monkeypatch):
...
async def test_kora_webhook_receives_and_records_once(monkeypatch):
    ...
    assert response == {"status": "received", "event_id": "evt-1"}
    assert db.records[0].event_id == "evt-1"
...
async def test_kora_webhook_duplicate_is_acknowledged_without_processing(monkeypatch):
    ...
    response = await kora_webhook(make_request(body, sign(data)), db)
    assert response == {"status": "already_received", "event_id": "evt-duplicate"}
